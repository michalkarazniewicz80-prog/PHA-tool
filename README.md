...
- Worksheet residual severity and likelihood remain manually adjustable by the facilitator/study team. Validated safeguards report only the highest single available safeguard credit; credits are not summed.
...
- Each worksheet row supports three safeguard entries, each paired with its own safeguard type and validation status. Only safeguards marked `Validated` are eligible for credit reporting.
